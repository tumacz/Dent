...
public class CreateAppointmentCommandValidator : AbstractValidator<CreateAppointmentCommand>
{
    ...
    {
        var service = await _serviceRepository.GetById(command.DentalStudioServiceId);
        if (service == null || service.Duration == null)
            return false;

        var endTime = command.StartTime.Add(service.Duration.Value);

        var existingAppointments = await _serviceRepository.GetAppointmentsForService(command.DentalStudioServiceId, command.StartTime.Date);
